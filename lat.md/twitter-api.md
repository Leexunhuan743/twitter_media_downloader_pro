...
## GraphQL Endpoints
Five core endpoints are used for user resolution, timeline fetching, and list operations.
...
## Error Types
Typed errors returned by the Twitter API layer, each with distinct handling semantics.

- `TwitterApiError` — API responded with error
- `ErrWouldBlock` — blocked by target user
- `ErrAccountLocked` — account temp-locked
- `ErrUserNotFound` — handle doesn't exist
- `ErrUserUnavailable` — suspended / deleted
- `ErrGuestToken` — guest token expired

## Protected Users

Protected (private) users require:
Users with private accounts require special handling since only followers can see their content.
