...
If no task flags are supplied, `Execute` logs a no-op hint and returns nil. A supplied `DownloadService` is used as-is for tests; otherwise the CLI creates one for the current run without mutating the dependencies object. SIGINT/SIGTERM cancel the CLI context.
