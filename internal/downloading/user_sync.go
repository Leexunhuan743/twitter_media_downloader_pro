...
func syncUserAndEntity(db *sqlx.DB, user *twitter.User, dir string, maxLen int) (*entity.UserEntity, error) {
	if err := database.SyncUser(db, user.Id, user.Name, user.ScreenName, user.IsProtected, user.FriendsCount, true); err != nil {
		log.Debugf("[download] User sync failed user=%q uid=%d error=%q", user.Title(), user.Id, err.Error())
		return nil, err
	}
	...
	if err != nil {
		log.Debugf("[download] User entity create failed user=%q uid=%d dir=%q error=%q", user.Title(), user.Id, logging.Path(dir), err.Error())
		return nil, err
	}
	if err = entity.Sync(ent, expectedTitle); err != nil {
		log.Debugf("[download] User entity sync failed user=%q uid=%d expected_title=%q error=%q", user.Title(), user.Id, expectedTitle, err.Error())
		return nil, err
	}
	return ent, nil
}
